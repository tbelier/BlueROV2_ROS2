...
{
    //std::ofstream donnee_x_y_z;
    //donnee_x_y_z.open("x_y_z.txt");
   ...
   /// MyDriver seatrac("/dev/ttyUSB0");
    ...
    //rclcpp::Node::SharedPtr nodeE_ = rclcpp::Node::make_shared("USBL_pub_nod");
    //rclcpp::Publisher<message::msg::Usbl>::SharedPtr pubE_ = nodeE_->create_publisher<message::msg::Usbl>("usbl", 1000);
    //seatrac.pub_ = pubE_;
    //seatrac.log.open("src/usbl_pub_cpp_pkg/logs/October_11_" + current_time() + ".dat");
    //seatrac.log << "LOG: northing, easting, depth, azimith, elevation, range, Local depth" << std::endl;

    //seatrac.donnee_x_y_z_driver.open("src/usbl_pub_cpp_pkg/x_y_z.txt");
    std::cout << "Avant 1er Ping" << std:: endl;

    command::ping_send(seatrac, BEACON_ID_15, MSG_REQU);
    std::cout << "ici" << std:: endl;
    getchar();
    //seatrac.donnee_x_y_z_driver.close();

    
    /*message::msg::Usbl lol;
    for(int i=0; i<= 10; ++i )
    {    
        ecriture(lol,donnee_x_y_z);
        sleep(1); // <-- ajout pour laisser un peu de temps entre 2 messages
    }
    donnee_x_y_z.close();*/
    ...
    //while(1) {
        ...
        try {
            ...
            //std::cout << "alive" << command::sys_alive(seatrac) << std::endl;
            //std::cout << "info" << command::sys_info(seatrac) << std::endl;
            //std::cout << "status_config_get" << command::status_config_get(seatrac) << std::endl;
            //std::cout << "setting_get" << command::settings_get(seatrac) << std::endl;
            std::cout << "ping_send" << command::ping_send(seatrac, BEACON_ID_3) << std::endl;
            //std::cout << command::ping_send(seatrac, BEACON_ID_15, MSG_REQU) << std::endl;
   ...
   /// ros::init(argc, argros2 header stamp cppv, "USBL_pub_node"); //
   
   
   /// ros::NodeHandle n;
   //rclcpp::Node node("USBL_pub_node");
   ...
   while (rclcpp::ok())
   {
       ...
       //seatrac.log << "LOG: no    std::time_t result = std::time(nullptr);
