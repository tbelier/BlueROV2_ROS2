...
class NicheVision(Node):
    def __init__(self):
        ...
        self.subscription = self.create_subscription(Image,'/ImageRaw',self.image_callback,10)
        self.imagePublisher = self.create_publisher(Image, '/ImageProcessed', 10)
        ...
        timerDelay = 1/60 #s
        self.timer = self.create_timer(timerDelay, self.image_publisher) #every 1 sec
        self.image = Image()
        

    def image_publisher(self):
        self.imagePublisher.publish(self.image)
        
    def image_callback(self, msg):
        frame = self.bridge.imgmsg_to_cv2(msg, 'bgr8')
        detected_image, Lposes = self.detect(frame, self.sizeMarker)
        self.image =  self.bridge.cv2_to_imgmsg(detected_image, 'bgr8')
