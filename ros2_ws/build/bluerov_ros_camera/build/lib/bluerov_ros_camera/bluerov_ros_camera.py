...
class BluerovRosCamera(Node):
    def __init__(self):
        ...
        self.video = Video(udp_port)
        self.publisher_ = self.create_publisher(Image, '/ImageRaw', 10)
        #self.publisher_ = self.create_publisher(Image, '/flir_boson/image', 10)
        #self.publisher_compressed = self.create_publisher(CompressedImage, '/bluerov_camera/compressed', 10)
        self.timer = self.create_timer(1/60, self.timer_callback)  # 20Hz
