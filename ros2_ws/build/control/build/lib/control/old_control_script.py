...
class Control(Node):
    def __init__(self):
        ...
        self.subscriptionPose = self.create_subscription(Pose, '/real/pose', self.callbackPose, 10)
        self.subscriptionDesiredPose = self.create_subscription(Pose, '/real/desiredPose', self.callbackDesiredPose, 10)

        # Publishers
        self.publisherU = self.create_publisher(Float64MultiArray, '/real/u', 10)
        
        # Callback timers
        timer_period = 0.5  # seconds
        self.timer = self.create_timer(timer_period, self.callbackPublisherU)
        ...
        self.set_mode('MANUAL') # also exist 'MANUAL', 'STABILIZE'
    ...
    # Messages from the topic for the REAL pose of the robot (sensors)
    def callbackPose(self,msg):
        self.x, self.y, self.z = msg.position
        rotation = R.from_quat(msg.orientation)
        self.yaw, self.pitch, self.roll = rotation.rotation.as_euler('xyz', degrees=False)

    # Messages from the topic for the DESIRED pose of the robot (guidance)
    def callbackDesiredPose(self,msg):
        self.xd, self.yd, self.zd = msg.position
        rotation = R.from_quat(msg.orientation)
        self.yawd, self.pitchd, self.rolld = rotation.rotation.as_euler('xyz', degrees=False)

    def callbackPublisherU(self):
        static_channels = [1500,1500,1500,1500,1500,1500,1500,1500,1100,1500,1500,1500,1100,1500,1500,1500]
        if self.isArmed:
            if self.isAutonomous:
                yaw   = int(1500+400*np.tanh(self.yawd-self.yaw))
                pitch = int(1500+400*np.tanh(self.pitchd-self.pitch))
                roll  = int(1500+400*np.tanh(self.rolld-self.roll))

                forward = int(1500+400*np.tanh(self.forwardd-self.forward))
                lateral = int(1500+400*np.tanh(self.laterald-self.lateral))
                ascend  = int(1500+400*np.tanh(self.ascendd-self.ascend))

                channels = [pitch,roll,ascend,yaw,forward,lateral,1500,1500,1100,1500,1500,1500,1100,1500,1500,1500]
